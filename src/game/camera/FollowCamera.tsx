...
// ---- Isometric elevation: ~35.264deg (arctan(1/sqrt2)) ----
const DISTANCE = 100
...
function FollowCamera({ playerYRef }: FollowCameraProps) {
  ...
  useFrame(() => {
    ...
    // ---- Use raycasted player Y from ref ----
    const playerY = playerYRef.current ?? 0

    // ---- Target position is player's position + isometric offset ----
    const targetX = playerPosition.x + INITIAL_X
    const targetY = playerY + INITIAL_Y
